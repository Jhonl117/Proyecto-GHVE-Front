...
const FONDOS_LIST = [
  'PORVENIR', 'COLPENCIONES', 'PROTECCION', 'SKANDIA'
];
...
const EditarEmpleado = () => {
  ...
  const cargarDatos = async () => {
    try {
      ...
      // Inicializar estados con los datos del empleado
      setCedula(data.cedula || "");
      setNombre_Completo(data.nombre_completo || "");
      setFecha_ingreso(data.fecha_ingreso ? data.fecha_ingreso.split('T')[0] : "");
      setFecha_nacimiento(data.fecha_nacimiento ? data.fecha_nacimiento.split('T')[0] : "");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      alerts.success("¡Actualizado!", "Los datos se guardaron correctamente.");
      ...
      if (Object.keys(newErrors).length > 0) {
        ...
        alerts.error("Error", error.response?.data?.msg || "No se pudo actualizar el empleado.");
      }
    } finally {
      setSaving(false);
    }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      >
        ...
        <div className="flex justify-between pt-6 border-t">
          <button type="button" onClick={() => navigate('/empleados/listaEmpleados')} className="flex items-center gap-2 px-6 py-2 text-gray-600 bg-gray-100 rounded-lg hover:bg-gray-200">
            <ArrowLeft className="w-4 h-4" /> Volver
          </button>
          <button type="submit" disabled={saving} className="flex items-center gap-2 px-8 py-2 text-white bg-primary-600 rounded-lg hover:bg-primary-700 disabled:opacity-50 font-bold">
